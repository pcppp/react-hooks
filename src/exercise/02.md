...
## 📝 Your Notes
useState中传入一个函数(函数本身，而不是函数的调用)可以避免重复的调用，只有在组件第一次渲染的时候调用这个函数

state本身是一个复杂对象，可能会导致不必要的更新，即使对象内容没有实际变化

使用JSON.stringify来确保只在有意义的变化时更新

自定义钩子是可以被复用的，每个钩子之间的状态都互相独立

组件的重新渲染实质上是函数的重新调用，调用的钩子函数也会重新执行，但是useState和useRef会有一个状态保持，在自定义钩子中重新调用这些钩子时仍有上次保存的值，并且在重复的调用时也保持独立。
